...
public class WebSocketServerHandler extends SimpleChannelInboundHandler {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        ctx.writeAndFlush("");
        Set<Map.Entry<Integer, ChannelGroup>> entries = channelGroupMap.entrySet();
        for (Map.Entry<Integer, ChannelGroup> entry : entries) {
    ...
    private void broadcast(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        String rep = ( name + request);
        System.out.println("income----------"+income.get(ctx.channel()));
        //Response response = MessageService.sendMessage(client, request);
        //String msg = new JSONObject(response).toString();
        List<String> msgs = jedis.lrange("msg", 0, -1);
        if (channelGroupMap.containsKey(client.getRoomId())) {

            channelGroupMap.get(client.getRoomId()).writeAndFlush(new TextWebSocketFrame(rep));
            jedis.lpush("msg",rep);
            //income.put(ctx.channel(),1);
